...
public class TrainCache {
    ...
    public ArrayList<TicketPojo> bookTrain(ArrayList<PassengerPojo> passenger) throws CustomException
    {
      ...
      System.out.println("the list is "+ passenger);
    	...
    	if(tick.getSeatType().equals("waiting list") && val <= waitLimit)
    	{
    		val++;
    		tick.setTicketConfirmation(false);
    		waitList.add(tick);
    	}
    	if(seatNumber > bookingLimit &&( pojo.getSeatTypes().equals("sideberth")))
        	{
        		
        		racList.add(tick);
        	}
        		if(racList.size() > racLimit && pojo.getSeatTypes().equals("waiting list"))
        		{
        			waitList.add(tick);
    ...
    {
    	System.out.println(bookTicketMap);
